...
public class CategoryService {
    ...
    public List<Category> listCategoriesUsedInForm() {
        List<Category> categriesUsedInForm = new ArrayList<>();

        Iterable<Category> categoriesInDB = categoryRepository.findRootCategories(Sort.by("name").ascending());
        for (Category category : categoriesInDB) {
            if (category.getParent() == null) {
                categriesUsedInForm.add(Category.copyIdAndName(category));

                Set<Category> children = sortSubCategories(category.getChildren());
                for (Category subCategory : children) {
                    String name = "--" + subCategory.getName();
                    categriesUsedInForm.add(Category.copyIdAndName(subCategory.getId(), name));
                    listSubCategoriesUsedInForm(categriesUsedInForm, subCategory, 1);
                }
            }
        }
        return categriesUsedInForm;
    }
    ...
    public void delete(Integer id) throws CategoryNotFoundException {
        Long countBytId = categoryRepository.countById(id);
